...
using Earth.Data.EF.Interfaces.UnitOfWork;
using Earth.Data.EF.Models;
using Microsoft.EntityFrameworkCore.ChangeTracking;

namespace Earth.Data.EF.Services.UnitOfWork
{
    public abstract class UnitOfWork<TDbContext> : IUnitOfWork where TDbContext : IDbContext
    {
        ...
        public List<T> ExecuteQuery<T>(string text, CommandType type = CommandType.Text,
            params SqlParameter[] parameters) where T : class, new()
        {
            return DbContext.ExecuteQuery<T>(text, type, parameters);
        }
